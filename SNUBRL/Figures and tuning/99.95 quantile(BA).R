## Estimation of 99.995 quantile response BA
...
train <- subset(data_ori, year == year.temp & month == month.temp)
train <- subset(train, select = -c(year, month, CNT, lon, lat))
...
Z_BA <- Z * ((1-0.95)/(1-0.99))^(gamma) * ((1-0.99)/(1-0.9995))^(gamma)
...
map("world", fill = T, col = "gray90", xlim = c(-125, -63), ylim = c(24, 51))
quilt.plot(loc, Z_BA, ny = 35, add = TRUE)
title("99.95% quantiles of log(1+BA)", cex.main = 1.7)
