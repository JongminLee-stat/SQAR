#### validation (QSAR with latitude explanatory variable)
...
for (year.temp in year){
  for (month.temp in month){
    ...
    for (i in c(4, 8:length(covariate))){
      ori <- train_DF_full[, i]
    ...
    test_DF_full <- subset(test_DF_full, select = -c(CNT, lon, year, month))
    test_DF <- test_DF_origin <- test_DF_full[missing, ]
    
   
    # missing imputation (Thin-plate spline, Tps)
    y <- train_DF$BA
    y <- as.matrix(y)
    y_log <- log(1 + y)
    ...
    train_DF <- train_imp
    train_DF <- subset(train_DF, select = -c(CNT, lon, year, month))
    # test set without BA
    ...
    for (j in 1:m){
      ...
      instru <- as.data.frame(W %*% X)  # instrumental variables
      names(instru) <- c("lat.inst", "area.inst", "lc1.inst", "lc2.inst", "lc3.inst", "lc4.inst", "lc5.inst", "lc6.inst", "lc7.inst", "lc8.inst", "lc9.inst",
...
#setwd("/Users/jongminlee/Desktop/EVA2021/Data")  # modify your working directory with the data file if necessary (using the setwd() command):
...
for (year.temp in year){
  for (month.temp in month){
    ...
    test_DF_full <- subset(test_DF_full, select = -c(BA, lon, year, month))
    test_DF <- test_DF_origin <- test_DF_full[missing, ]
    
    
    ## missing values imputation (Thin-plate spline, Tps)
    y <- train_DF$CNT
    y <- as.matrix(y)
    ...
    # train with imputation
    train_DF <- train_imp
    train_DF <- subset(train_DF, select = -c(BA, lon, year, month))
