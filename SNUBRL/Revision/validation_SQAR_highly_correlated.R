## validation score (SQAR with much correlated BA and CNT)
...
for (year.temp in year){
  for (month.temp in month){
    ...
    missing_temp <- is.na(train_DF_full$CNT)
    loc_obs <- loc[!missing_temp, ]
    loc_miss <- loc[missing_temp, ]
    
    # CNT imputation using Thin-plate spline (TPS)
    y_temp <- train_DF_full$CNT[!missing_temp]
    y_temp <- as.matrix(y_temp)
    z_temp <- y_temp + runif(nrow(y_temp), 0, 0.999)
    #z_temp <- sqrt(y_temp + 3/8)
    fit <- Tps(loc_obs, z_temp, lambda = 0, lon.lat = TRUE, miles = FALSE)
    impute <- floor(predict(fit, loc_miss))
    #impute <- floor((predict(fit, loc_miss))^2 - 1/8)
    for (i in 1:nrow(impute)){ # If impute value is negative, set 0.
      if (impute[i] < 0){
        impute[i] <- 0
      }
    }
    train_DF_full$CNT[missing_temp] <- impute
    
    # covariate scaling
    covariate <- names(train_DF_full)
    for (i in c(1, 8:length(covariate))){
      ori <- train_DF_full[, i]
    ...
    test_DF_full <- subset(test_DF_full, select = -c(lon, lat, year, month))
    test_DF <- test_DF_origin <- test_DF_full[missing, ]
    
    # missing location using Thin-plate spline (TPS)
    y <- train_DF$BA
    y <- as.matrix(y)
    y_log <- log(1 + y)
    ...
    train_DF <- train_imp
    train_DF <- subset(train_DF, select = -c(lon, lat, year, month))
    # test set without BA
    ...
    for (j in 1:m){
      ...
      train_DF <- rename(train_DF, "WZ" = "BA")
      train_DF$WZ <- W %*% Z
      X <- as.matrix(train_DF[, -2])
      instru <- as.data.frame(W %*% X)  # instrumental variable
      names(instru) <- c("CNT.inst", "area.inst", "lc1.inst", "lc2.inst", "lc3.inst", "lc4.inst", "lc5.inst", "lc6.inst", "lc7.inst", "lc8.inst", "lc9.inst",
...
for (year.temp in year){
  for (month.temp in month){
    ...
    # BA imputation (Thin-plate spline)
    missing_temp <- is.na(train_DF_full$BA)
    loc_obs <- loc[!missing_temp, ]
    loc_miss <- loc[missing_temp, ]
    y_temp <- train_DF_full$BA[!missing_temp]
    z_temp <- log(1 + as.matrix(y_temp))
    fit <- Tps(loc_obs, z_temp, lambda = 0, lon.lat = TRUE, miles = FALSE)
    impute <- exp(predict(fit, loc_miss)) - 1
    for (i in 1:nrow(impute)){ # If impute value is negative, set 0.
      if (impute[i] < 0){
        impute[i] <- 0
      }
    }
    train_DF_full$BA[missing_temp] <- impute
    
    # covariate scaling
    covariate <- names(train_DF_full)
    for (i in c(2, 8:length(covariate))){
      ori <- train_DF_full[, i]
      scaled <- (ori - mean(ori))/sd(ori)
      train_DF_full[, i] <- scaled
    }
    ...
    ## CNT prediction
    Y <- train_DF$CNT
    Z <- Y + runif(length(Y), 0, 0.999) # jittering (discrete -> continuous)
    #Z <- sqrt(Y + 3/8) # Poissonization (Anscombe transform)
    taus <- seq(0.01, 0.95, by = 0.01)
    m <- length(taus)
    pred <- matrix(nr = nrow(test_DF), nc = m)   # estimated quantiles
    ...
    for (j in 1:m){
      ...
      instru <- as.data.frame(W %*% X)  # instrumental variable
      names(instru) <- c("BA.inst", "area.inst", "lc1.inst", "lc2.inst", "lc3.inst", "lc4.inst", "lc5.inst", "lc6.inst", "lc7.inst", "lc8.inst", "lc9.inst",
                         "lc10.inst", "lc11.inst", "lc12.inst", "lc13.inst", "lc14.inst", "lc15.inst", "lc16.inst", "lc17.inst", "lc18.inst",
                         "altiMean.inst", "altiSD.inst", "clim1.inst", "clim2.inst", "clim3.inst", "clim4.inst", "clim5.inst", "clim6.inst",
                         "clim7.inst", "clim8.inst", "clim9.inst", "clim10.inst")
      train_with_instru <- cbind(train_DF, instru)
