## validation score (SQAR with exponential power decay alpha = 0.5)
...
alpha <- 2       # exponential power decay   
for (i in 2:n){  
  for (j in 1:(i - 1)){
    dist <- distGeo(loc[i, ], loc[j, ])/1000  # distance between ith and jth locations. (km)
    if (dist < 800){
      z[i, j] <- exp(-(dist/scale)^(alpha))   # exponential power decay
      #z[i, j] <- (dist/scale)^(-alpha)       # power decay
    } else {
      z[i, j] <- 0
    }
    z[j, i] <- z[i, j]
...
print("Validation score of BA (polynomial alpha=0.5) is")
sum(ba.score)
...
print("Validation score of CNT (exponential polynomial power alpha = 0.5) is")
sum(cnt.score)

#### get validation score of BA
print("Validation score of BA (exponential polynomial power alpha = 0.5) is")
sum(ba.score)
